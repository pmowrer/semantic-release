...
test('Maintenance branches - initial state', t => {
  ...
  t.deepEqual(
    normalize
      ...
      .map(({type, name, range, channel, 'merge-range': maintenanceRange}) => ({
        type,
        name,
        range,
        channel,
    ...
    [
      {type: 'maintenance', name: '1.1.x', range: '>=1.1.0 <1.0.0', channel: '1.1.x', 'merge-range': '>=1.1.0 <1.2.0'},
      {type: 'maintenance', name: '1.2.x', range: '>=1.2.0 <1.0.0', channel: '1.2.x', 'merge-range': '>=1.2.0 <1.3.0'},
      {type: 'maintenance', name: '1.x', range: '>=1.3.0 <1.0.0', channel: '1.x', 'merge-range': '>=1.3.0 <2.0.0'},
    ]
  );
});
...
test('Maintenance branches - cap range to first release present on default branch and not in any Maintenance one', t => {
  ...
  t.deepEqual(
    ...
    [
      {type: 'maintenance', name: 'name', range: '>=1.1.1 <1.2.0', channel: 'name', 'merge-range': '>=1.1.0 <1.2.0'},
      {type: 'maintenance', name: '1.2.x', range: '>=1.2.1 <1.3.0', channel: '1.2.x', 'merge-range': '>=1.2.0 <1.3.0'},
      {type: 'maintenance', name: '1.x', range: '>=1.5.0 <1.6.0', channel: '1.x', 'merge-range': '>=1.3.0 <2.0.0'},
      {type: 'maintenance', name: '2.x.x', range: '>=2.0.0 <1.6.0', channel: '2.x.x', 'merge-range': '>=2.0.0 <3.0.0'},
    ]
  );
});
...
test('Maintenance branches - cap range to default branch last release if all release are also present on maintenance branch', t => {
  ...
  t.deepEqual(
    ...
    [
      {type: 'maintenance', name: '1.x', range: '>=1.3.0 <2.0.0', channel: '1.x', 'merge-range': '>=1.0.0 <2.0.0'},
      {type: 'maintenance', name: '2.x.x', range: '>=2.0.0 <2.0.0', channel: '2.x.x', 'merge-range': '>=2.0.0 <3.0.0'},
    ]
  );
});
